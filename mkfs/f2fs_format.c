...
static inline u_int32_t BIT_ENTRY_COUNT() { return get_sb(section_count_meta); }
...
static int init_mm_info() {
  ...
  mmi.current_secno = (get_sb(sit_blkaddr) - get_sb(segment0_blkaddr)) /
                      (1 << get_sb(log_blocks_per_seg)) / get_sb(segs_per_sec);
  ...
  mmi.block_information_table =
      calloc(CEILING(BIT_ENTRY_COUNT(), 1024), sizeof(u_int32_t));
...
static int f2fs_prepare_super_block(void) {
  ...
  zone_size_bytes =
      blk_size_bytes * c.secs_per_zone * c.segs_per_sec * c.blks_per_seg;

  set_sb(checksum_offset, 0);

  set_sb(block_count, c.total_sectors >> log_sectors_per_block);

  zone_align_start_offset = ((u_int64_t)c.start_sector * DEFAULT_SECTOR_SIZE +
  ...
  if (c.zoned_mode &&
      ((c.ndevs == 1 && (get_sb(segment0_blkaddr) +
                         c.start_sector / DEFAULT_SECTORS_PER_BLOCK) %
                            c.zone_blocks) ||
       (c.ndevs > 1 && c.devices[1].start_blkaddr % c.zone_blocks))) {
  ...
  for (i = 0; i < c.ndevs; i++) {
    if (i == 0) {
      c.devices[i].total_segments =
          (c.devices[i].total_sectors * c.sector_size -
           zone_align_start_offset) /
          segment_size_bytes;
      c.devices[i].start_blkaddr = 0;
      c.devices[i].end_blkaddr = c.devices[i].total_segments * c.blks_per_seg -
                                 1 + sb->segment0_blkaddr;
    } else {
      c.devices[i].total_segments =
      ...
      c.devices[i].end_blkaddr = c.devices[i].start_blkaddr +
                                 c.devices[i].total_segments * c.blks_per_seg -
                                 1;
    }
    if (c.ndevs > 1) {
  ...
  set_sb(nat_blkaddr,
         get_sb(sit_blkaddr) + get_sb(segment_count_sit) * c.blks_per_seg);
  ...
  if (c.large_nat_bitmap) {
    ...
    if (max_sit_bitmap_size + max_nat_bitmap_size > MAX_BITMAP_SIZE_IN_CKPT) {
      u_int32_t diff =
          max_sit_bitmap_size + max_nat_bitmap_size - MAX_BITMAP_SIZE_IN_CKPT;
  ...
  total_meta_segments = (get_sb(segment_count_sit) + get_sb(segment_count_nat) +
  ...
  if (diff) {
    set_sb(segment_count_ssa,
           get_sb(segment_count_ssa) + (c.segs_per_zone - diff));
  ...
  set_sb(last_ssa_blkaddr,
         (get_sb(ssa_blkaddr) + get_sb(segment_count_ssa) * c.blks_per_seg) -
             1);
  ...
  set_sb(main_blkaddr,
         get_sb(sit_blkaddr) +
             ((total_meta_zones)*c.segs_per_zone * c.blks_per_seg));
  ...
  if ((!(c.feature & cpu_to_le32(F2FS_FEATURE_RO)) && c.overprovision == 0) ||
  ...
  utf8_to_utf16(sb->volume_name, (const char *)c.vol_label, MAX_VOLUME_NAME,
                strlen(c.vol_label));
  set_sb(node_ino, 1);
  set_sb(meta_ino, 2);
  set_sb(root_ino, 3);
...
static int f2fs_write_check_point_pack(void) {
  ...
  nat_bits_blocks =
      F2FS_BYTES_TO_BLK((nat_bits_bytes << 1) + 8 + F2FS_BLKSIZE - 1);
  ...
  set_cp(valid_block_count,
         2 + c.quota_inum + c.quota_dnum + c.lpf_inum + c.lpf_dnum);
  ...
  set_cp(overprov_segment_count,
         (f2fs_get_usable_segments(sb) - get_cp(rsvd_segment_count)) *
             c.overprovision / 100);
  set_cp(overprov_segment_count,
  ...
  if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) {
    ...
    set_cp(user_block_count,
           ((get_cp(free_segment_count) + 2 - get_cp(overprov_segment_count)) *
            c.blks_per_seg));
  } else {
    set_cp(free_segment_count, f2fs_get_usable_segments(sb) - 6);
    set_cp(user_block_count,
           ((get_cp(free_segment_count) + 6 - get_cp(overprov_segment_count)) *
            c.blks_per_seg));
  }
  /* cp page (2), data summaries (1), node summaries (3) */
  set_cp(cp_pack_total_block_count, 6 + get_sb(cp_payload) +
                                        CEILING(BAT_ENTRY_COUNT(), 1024) +
                                        CEILING(BIT_ENTRY_COUNT(), 1024) + 1);

  flags = CP_UMOUNT_FLAG | CP_COMPACT_SUM_FLAG;

  // add offsets
  set_cp(cp_pack_start_meta_bat, 1 + get_sb(cp_payload));
  set_cp(cp_pack_start_meta_bit,
         get_cp(cp_pack_start_meta_bat) + CEILING(BAT_ENTRY_COUNT(), 1024));
  set_cp(cp_pack_start_meta_bitmap,
         get_cp(cp_pack_start_meta_bit) + CEILING(BIT_ENTRY_COUNT(), 1024));
  ...
  *((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
  ...
  for (qtype = 0, i = 1; qtype < F2FS_MAX_QUOTAS; qtype++) {
    ...
    journal->nat_j.entries[i].ne.block_addr = cpu_to_le32(
        get_sb(main_blkaddr) + get_cp(cur_node_segno[0]) * c.blks_per_seg + i);
    i++;
  }
  ...
  for (i = 0; i < BIT_ENTRY_COUNT(); i += 1024) {
    if (dev_write_block(mmi.block_information_table + i, cp_seg_blk++)) {
      MSG(1, "\tError: while writing the mmi.block_information_table to "
             "disk!!!\n");
      goto free_cp_payload;
    }
  }
  ...
  if (dev_write(mmi.section_bitmap, cp_seg_blk << F2FS_BLKSIZE_BITS,
                SECTION_BITMAP_SIZE)) {
  ...
  if (flags & CP_NAT_BITS_FLAG) {
    ...
    *(__le64 *)nat_bits = get_cp_crc(cp);
    ...
    cp_seg_blk = get_sb(segment0_blkaddr) + (1 << get_sb(log_blocks_per_seg)) -
                 nat_bits_blocks;
  ...
  *((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
  ...
  DBG(1,
      "\tWriting cp page 1 of checkpoint pack 2, at offset 0x%08" PRIx64 "\n",
      cp_seg_blk);
  if (dev_write_block(cp, cp_seg_blk)) {
    MSG(1, "\tError: While writing the cp to disk!!!\n");
    goto free_cp_payload;
  ...
  cp_seg_blk +=
      (le32_to_cpu(cp->cp_pack_total_block_count) - get_sb(cp_payload) - 1);
  DBG(1,
      "\tWriting cp page 2 of checkpoint pack 2, at offset 0x%08" PRIx64 "\n",
      cp_seg_blk);
  if (dev_write_block(cp, cp_seg_blk)) {
    MSG(1, "\tError: While writing the cp to disk!!!\n");
    goto free_cp_payload;
...
static int f2fs_discard_obsolete_dnode(void) {
  ...
  u64 end_blkaddr = (get_sb(segment_count_main) << get_sb(log_blocks_per_seg)) +
                    get_sb(main_blkaddr);
...
static int f2fs_write_root_inode(void) {
  ...
  raw_node->footer.next_blkaddr = cpu_to_le32(
      get_sb(main_blkaddr) + c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg + 1);
  ...
  data_blk_nor =
      get_sb(main_blkaddr) + c.cur_seg[CURSEG_HOT_DATA] * c.blks_per_seg;
  ...
  DBG(1,
      "\tWriting root inode (hot node), %u %x %x at offset 0x%08" PRIu64 "\n",
...
static int f2fs_write_qf_inode(int qtype) {
  ...
  raw_node->footer.next_blkaddr =
      cpu_to_le32(get_sb(main_blkaddr) +
                  c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg + 1 + qtype + 1);
  raw_node->i.i_blocks = cpu_to_le64(1 + QUOTA_DATA(qtype));

  data_blk_nor =
      get_sb(main_blkaddr) + c.cur_seg[CURSEG_HOT_DATA] * c.blks_per_seg + 1;
  ...
  main_area_node_seg_blk_offset = get_sb(main_blkaddr);
  main_area_node_seg_blk_offset +=
      c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg + qtype + 1;

  DBG(1,
      "\tWriting quota inode (hot node), %x %x %x at offset 0x%08" PRIu64 "\n",
      get_sb(main_blkaddr), c.cur_seg[CURSEG_HOT_NODE], c.blks_per_seg,
      main_area_node_seg_blk_offset);
  if (write_inode(raw_node, main_area_node_seg_blk_offset) < 0) {
    MSG(1, "\tError: While writing the raw_node to disk!!!\n");
...
static int f2fs_update_nat_root(void) {
  ...
  for (qtype = i = 0; qtype < F2FS_MAX_QUOTAS; qtype++) {
    ...
    nat_blk->entries[sb->qf_ino[qtype]].block_addr =
        cpu_to_le32(get_sb(main_blkaddr) +
                    c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg + i + 1);
  ...
  meta_blk->bat_chunk[nat_seg_blk_idx % BAT_CHUNK_SIZE] =
      cpu_to_le32(mmi.current_wp);
...
static block_t f2fs_add_default_dentry_lpf(void) {
  ...
  data_blk_offset +=
      c.cur_seg[CURSEG_HOT_DATA] * c.blks_per_seg + 1 + c.quota_dnum;
...
static int f2fs_write_lpf_inode(void) {
  ...
  main_area_node_seg_blk_offset = get_sb(main_blkaddr);
  main_area_node_seg_blk_offset +=
      c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg + c.quota_inum + 1;

  DBG(1,
      "\tWriting lost+found inode (hot node), %x %x %x at offset 0x%08" PRIu64
      "\n",
      get_sb(main_blkaddr), c.cur_seg[CURSEG_HOT_NODE], c.blks_per_seg,
      main_area_node_seg_blk_offset);
  if (write_inode(raw_node, main_area_node_seg_blk_offset) < 0) {
